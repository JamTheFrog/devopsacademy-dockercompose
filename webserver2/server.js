...
app.use(async (req, res, next) => {
  try {
    const webserver2Count = await client.incr('webserver2_count');
    const totalVisits = await client.incr('total_visits');
    
    res.locals.webserver2Count = webserver2Count;
...
app.get("/", (req, res) => {
  const greeting = `Webserver 2 is running on http://localhost:${PORT}`;
  res.send(`
    <h1>${greeting}</h1>
    <p>You have visited webserver2: ${res.locals.webserver2Count} times.</p>
    <p>You have visited both webservers: ${res.locals.totalVisits} times.</p>
  `);
});

app.listen(PORT, () => {
  console.log(`Webserver 2 is running on http://localhost:${PORT}`);
});
